...
def A_Star_Search(start_pos, goal_pos):           #return next move
   ...
   while len(white_list) > 0:
      ...
      #found the goal_node
      if gray_node.compare(goal_node):
          path = []
          current = gray_node
      ...
      children = []
      for move_index, move_to in enumerate([(25, 0), (25, 0), (0, -25), (0, 25)]):    # 5 points per time step
        ...
        if(len(next_step) == 0):      # trigger escape but all escaping direction hit the wall
            if(x_between == 0):           # final escaping method: turn left or right
                ...
                if(not hit_the_wall(playerState[0], playerState[1], 3)):
                    next_step.append(3)
        return next_step
    #the ghost is near but not chasing us, don't worry
    else:
        return next_step
